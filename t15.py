...
def downmodel(model_name, model_id, model_dir):
    ...
    # Get the file format from the API response
    file_format = response.json().get('modelVersion').get('files')[0].get('format')

    # Add the appropriate file extension based on the file format
    if file_format == 'pickle':
        model_path += '.pickle'
    elif file_format == 'safetensor':
        model_path += '.safetensors'
    elif file_format == 'ckpt':
        model_path += '.ckpt'
...
model_dir = r'C:\Users\FOX\stable-diffusion-webui\models\Stable-diffusion'
